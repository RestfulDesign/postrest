...
module.exports = (function () {
    ...
    function reply(resolver, data, response) {
        ...
        message.server.status = response ? (response.statusCode || response.status) : -1;
        message.server.headers = response && response.hasOwnProperty('headers') ? response.headers : {};
        ...
        // success
        if(message.server.status > 0 &&
            message.server.status < 400) {
        ...
        return resolver.reject(message);
    }
